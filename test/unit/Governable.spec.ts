...
import { fnShouldOnlyBeCallableByGovernor, fnShouldOnlyBeCallableByPendingGovernor } from '@utils/behaviours';
...
describe('Governable', () => {
  ...
  describe('constructor', () => {
    it('should revert when given zero address', async () => {
      await expect(governableFactory.deploy(ZERO_ADDRESS)).to.be.revertedWith('NoGovernorZeroAddress()');
    });
  });
  ...
  context('after deployed', () => {
    ...
    describe('setPendingGovernor', () => {
      ...
      it('should revert when given zero address', async () => {
        await expect(governable.connect(governor).setPendingGovernor(ZERO_ADDRESS)).to.be.revertedWith('NoGovernorZeroAddress()');
      });
    ...
    describe('acceptPendingGovernor', () => {
      ...
      fnShouldOnlyBeCallableByPendingGovernor(
        () => governable,
